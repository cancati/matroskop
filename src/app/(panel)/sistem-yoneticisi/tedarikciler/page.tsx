...
import { Search, Building2, Edit, ExternalLink, Plus, ChevronLeft, ChevronRight } from "lucide-react"
import { TedarikciModal } from "@/components/sistem-yoneticisi/TedarikciModal"
import type { SupplierWithCount, PaginatedResponse } from "@/types/admin"
...
export default function TedarikcilerPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/tedarikciler?${params}`)
      if (!res.ok) throw new Error("Veri yuklenemedi.")
      const json: PaginatedResponse<SupplierWithCount> = await res.json()
  ...
  const openEdit = (s: SupplierWithCount) => { setEditingSupplier(s); setShowModal(true) }

  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-brand">Tedarikciler</h1>
          <p className="text-muted text-[14px] mt-0.5">{total} tedarikci kayitli</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-surface-section">
              <tr>
                {["Tedarikci Adi", "Telefon", "Adres", "Ogrenci", "Sorumlu", "Durum", "Islemler"].map((h) => (
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <Building2 className="w-10 h-10 text-muted/30" />
                      <p className="text-muted text-[14px]">Hicbir tedarikci bulunamadi.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                suppliers.map((s) => (
                  <tr key={s.id} className="border-t border-form-border hover:bg-surface/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-muted text-[14px] max-w-[180px] truncate">{s.address ?? "—"}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(s)} className="text-muted hover:text-brand transition-colors p-1"><Edit className="w-4 h-4" /></button>
                        <Link href={`/sistem-yoneticisi/tedarikciler/${s.id}`} className="text-muted hover:text-brand transition-colors p-1"><ExternalLink className="w-4 h-4" /></Link>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      <TedarikciModal isOpen={showModal} onClose={() => setShowModal(false)} tedarikci={editingSupplier} onSuccess={load} />
    </div>
  )
}
