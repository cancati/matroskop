...
export default function OkulRaporlarPage() {
  ...
  useEffect(() => {
    fetch("/api/okul-yoneticisi/raporlar")
      .then((r) => r.json())
      .then(setReports)
  ...
  if (loading) {
    return <div className="flex flex-col gap-4">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="animate-pulse bg-brand-light rounded-2xl h-20" />)}</div>
  }
  if (error) {
    return <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-red-700 text-[14px]">{error}</div>
  }

  const withExams = reports.filter((r) => r.exams.length > 0)
  const withoutExams = reports.filter((r) => r.exams.length === 0)

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold text-brand">Raporlar</h1>
        <p className="text-muted text-[14px] mt-0.5">{reports.length} ogrenci · {withExams.length} tamamlanmis sinav</p>
      </div>
      ...
      {reports.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {[...withExams, ...withoutExams].map((student) => {
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="text-left">
                      ...
                      <p className="text-[12px] text-muted">{student.classroom ?? "Sinifsiz"} · <span className="font-mono">{student.username}</span></p>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    {lastExam?.result && (
                      <>
                        ...
                        <span className="text-[15px] font-bold text-brand tabular-nums w-12 text-right">{lastExam.result.G76.toFixed(1)}</span>
                      </>
                    )}
                    {student.exams.length === 0 && <span className="text-muted text-[13px]">Sinav yok</span>}
                ...
                {isOpen && (
                  <div className="border-t border-form-border divide-y divide-form-border">
                    {student.exams.length === 0 ? (
                      <div className="px-6 py-6 text-center text-muted text-[14px]">Henuz tamamlanmis sinav bulunmuyor.</div>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {exam.result && (
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-[18px] font-black text-brand tabular-nums">{exam.result.G76.toFixed(1)}</span>
                            </div>
                          )}
                        </div>
                        {exam.result && (
