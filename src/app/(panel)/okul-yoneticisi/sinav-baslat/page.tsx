...
export default function OkulSinavBaslatPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch("/api/okul-yoneticisi/ogrenciler")
  ...
  const createExam = async (studentId: string, studentName: string) => {
    ...
    setError(null)
    try {
      const res = await fetch("/api/okul-yoneticisi/sinav-olustur", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setSuccess(`${studentName} icin sinav olusturuldu.`)
        await load()
      }
    } catch {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted text-[14px] mt-0.5">Ogrenci icin sinav olusturun. Ogrenci giris yaptiginda sinavi gorecektir.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-form-border overflow-hidden">
        <table className="w-full">
          <thead className="bg-surface-section">
            <tr>
              {["Ogrenci", "Sinif", "Sinav Durumu", "Islem"].map((h) => (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted text-[14px]">Kayitli ogrenci bulunamadı.</p>
                  </div>
                </td>
              </tr>
            ) : students.map((s) => (
